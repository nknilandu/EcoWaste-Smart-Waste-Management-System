...
export default function ProfilePage() {
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    if (avatar) {
      ...
      const { error: uploadErr } = await supabase.storage.from('avatars').upload(path, avatar, { upsert: true });
      if (!uploadErr) {
    ...
    const { error } = await supabase.from('profiles').update({ full_name: name, avatar_url: avatarUrl }).eq('user_id', user.id);
    if (error) {
  ...
  return (
    <div className="min-h-screen bg-background py-8 px-4">
      <div className="max-w-lg mx-auto animate-fade-in-up">
        ...
        <form onSubmit={handleSave} className="bg-card border rounded-2xl p-8 space-y-8">
          {/* Avatar */}
          ...
          <div className="space-y-5">
            ...
            <div>
              <label className="block text-sm font-medium mb-1.5 flex items-center gap-1.5"><Mail className="h-3.5 w-3.5" /> Email</label>
              <input type="email" value={user?.email || ''} disabled
                className="w-full px-4 py-3 border rounded-xl bg-muted text-muted-foreground text-sm" />
            </div>
            <div>
              <label className="block text-sm font-medium mb-1.5 flex items-center gap-1.5"><Shield className="h-3.5 w-3.5" /> Role</label>
              <input type="text" value={role || 'citizen'} disabled
